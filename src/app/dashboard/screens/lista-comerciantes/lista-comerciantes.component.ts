...
export default class ListaComerciantesComponent implements OnInit {
  ...
  nextElementos(): void {
    ...
    var aux1 = this.viewComerciantesComercial[this.viewComerciantesComercial.length - 1];
    ...
    if ((aux2 + 1) + 7 > this.comerciantesService.listaComerciante().length) {
      ...
      if (aux3 != 0) {
        this.viewComerciantesComercial = this.comerciantesService.listaComerciante().slice(aux3);
      }
      return;
    }
    this.viewComerciantesComercial = this.comerciantesService.listaComerciante().slice(aux2 + 1, (aux2 + 1) + 7);
  }
  ...
  backElementos(): void {
    ...
    if (aux3 >= 0) {
      ...
      this.viewComerciantesComercial = this.comerciantesService.listaComerciante().slice(aux2 - 7, aux2);
    }
  }
